    ...
    return (
        <header style={{ background: '#FFF9E2' }}>
          ...
          style={{ cursor: 'pointer' }}
                                    />
    {searchVisible && (
        <div className={`search-container ${searchVisible ? 'active' : ''} absolute top-16 right-10 bg-white p-4 shadow-lg rounded-full`}>
          <form onSubmit={handleSearch} className="relative">
            <input
              type="text"
              id="search-field"
              className="search-field p-2 pl-10 border border-gray-300 rounded-full"
              placeholder=" "
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <label htmlFor="search-field" className="search-label absolute left-3 top-2/4 transform -translate-y-2/4 text-gray-500">
              Search
            </label>
            <button type="submit" className="search-button absolute right-3 top-2/4 transform -translate-y-2/4 bg-transparent border-none cursor-pointer">
